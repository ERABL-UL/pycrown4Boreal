...
def clean(workdir, tif_ext, las_ext):
    if os.path.exists("{}{}{}".format(workdir, '/all', las_ext)):
        os.remove(workdir + '/all' + las_ext)
    if os.path.exists("{}{}{}".format(workdir, '/POINTS', las_ext)):
        os.remove(workdir + '/POINTS' + las_ext)
    if os.path.exists("{}{}{}".format(workdir, '/ground', las_ext)):
        os.remove(workdir + '/ground' + las_ext)
    if os.path.exists("{}{}{}".format(workdir, '/CHM', tif_ext)):
...
def main(file, increment, verbose=False):
    cell_size = 0.25
    workdir = "/home/willalbert/Documents/GitHub/pycrown/example/data"
    ...
    try:
        ...
        if verbose:
            print("CORRUPTED FILE\n   INCREMENT: {}\n   FILE NAME: {}".format(increment, file))
        return
    ...
    to_extract = [1, 2]
    F_LAS_ALL = ply2las(points, to_extract, workdir + '/all' + las_ext, 0)
    to_extract = 1
    F_LAS_POINTS = ply2las(points, to_extract, workdir + '/POINTS' + las_ext, F_LAS_ALL.header.offset)
    to_extract = 2
    F_LAS_GROUND = ply2las(points, to_extract, workdir + '/ground' + las_ext, F_LAS_ALL.header.offset)

    if F_LAS_GROUND == 0 or F_LAS_POINTS == 0:
        if verbose:
            print("NO GROUND POINTS OR TREE POINTS")
            print("LAS GROUND POINTS FILE: {}".format(F_LAS_GROUND))
            print("LAS TREE POINTS FILE: {}".format(F_LAS_POINTS))
            print(increment)
            print("=======================================")
    ...
    PC.filter_chm(8, ws_in_pixels=True)

    # Tree Detection with local maximum filter
    returned = PC.tree_detection(PC.chm, ws=8, ws_in_pixels=True, hmin=2.)
    if returned == 0:
        if verbose:
    ...
    # Crown Delineation PARAMETERS DE PY CROWN : A ADAPTER ############################################## th_tree:
    # Mis à la valeur de 2. Threshold below which a pixel cannot be a tree. Default 2; th_seed: Valeur était à 0.45,
    # modifié à 0.4Growing threshold 1. A pixel is added to a region if its height is greater than the tree height
    # multiplied by this value. It should be between 0 and 1. Default 0.45"; th_crown: Valeur était à 0.55,
    # modifié à 0.5 Growing threshold 2. A pixel is added to a region if its height is greater than the current mean
    # height of the region multiplied by this value. It should be between 0 and 1. Default 0.55."; max_crown: Laissé
    # à la valeur de 10. "Maximum value of the crown diameter of a detected tree (in pixels). Default 10".
    PC.crown_delineation(algorithm='dalponteCIRC_numba', th_tree=2., th_seed=0.3, th_crown=0.2, max_crown=36)  #th_tree=4., th_seed=0.3, th_crown=0.2, max_crown=36
...
if __name__ == '__main__':
    ...
    increment = 0
    arglist = []
    for folder in ost.getDirBySubstr("/home/willalbert/lidar_mont_gosford_processed", "18_27"):
        for file in ost.getFileByExt(folder, "ply"):
            arglist.append([file, increment])
            increment += 1

    arglist = arglist[64400:]
    results = []
